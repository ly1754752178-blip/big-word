...
- 源文件：`F:\AI\gongcheng\孤独摇滚1.mp4`、`孤独摇滚2.mp4`、`孤独摇滚3.mp4`
- 复制到 `public/videos/` 目录
- `<video>` 标签，`object-fit: cover`，铺满全屏
- 叠加半透明暗色遮罩确保文字可读
- 默认每 60 秒自动切换到下一个视频
- 支持循环播放
...
- 播放/暂停、上一曲/下一曲、音量滑块、当前序号（如 1/3）
- 半透明磨砂玻璃风格，悬停时完全显示
- 纯前端实现，不依赖任何外部播放器库

### 3.4 左侧菜单

- 竖排罗列 5 个选项，3A 大作经典主菜单风格
- 选中/悬停高亮（发光边框）
- 「开始游戏」：
  - 无对话时 → 直接创建新对话并进入游戏
  - 有对话时 → 弹出对话列表供选择
...
| 操作 | 路径 |
|---|---|
| 复制视频 | `F:\AI\gongcheng\孤独摇滚*.mp4` → `public/videos/` |
| 新建目录 | `public/juesetouxiang/`（用户自行放入头像） |
